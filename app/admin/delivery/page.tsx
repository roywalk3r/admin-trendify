...
export default function AdminDeliveryPage() {
  ...
  const addCity = async () => {
    ...
    try {
      const res = await fetch("/api/admin/delivery/cities", { method: "POST", body: JSON.stringify({ name, doorFee }) })
      const data = await res.json()
  ...
  const updateCity = async (id: string, patch: Partial<City>) => {
    try {
      const res = await fetch(`/api/admin/delivery/cities/${id}`, { method: "PATCH", body: JSON.stringify(patch) })
      const data = await res.json()
  ...
  const addLocation = async (cityId: string, name: string, address?: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/delivery/locations`, { method: "POST", body: JSON.stringify({ cityId, name: n, address }) })
      const data = await res.json()
  ...
  const updateLocation = async (id: string, patch: Partial<Location>) => {
    try {
      const res = await fetch(`/api/admin/delivery/locations/${id}`, { method: "PATCH", body: JSON.stringify(patch) })
      const data = await res.json()
