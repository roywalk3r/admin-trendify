...
export async function GET(req: NextRequest) { try {
  ...
  const res = await fetch(`${origin}/api/returns`, { headers: { Authorization: req.headers.get("authorization") || "" }, cache: "no-store" })
...
export async function POST(req: NextRequest) { try {
  ...
  const res = await fetch(`${origin}/api/returns`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: req.headers.get("authorization") || "" }, body: JSON.stringify(body), cache: "no-store" })
  const json = await res.json().catch(() => ({}))
  const out = createApiResponse({ status: res.status, data: json?.data ?? json, error: json?.error })
  return withCors(out, req, { credentials: true })
} catch (e) { const out = handleApiError(e); return withCors(out, req, { credentials: true }) } }
