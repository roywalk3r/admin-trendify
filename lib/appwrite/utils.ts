...
export async function getMediaFiles(): Promise<MediaFile[]> {
  try {
    const response = await storage.listFiles(BUCKET_ID, [Query.orderDesc("$createdAt"), Query.limit(100)])
    ...
    return response.files.map((file) => ({
      ...
      preview: file.mimeType.startsWith("image/")
        ? storage.getFilePreview(BUCKET_ID, file.$id, 300, 300).toString()
        : undefined,
    }))
  } catch (error) {
...
export async function getFilePreview(fileId: string, width = 300, height = 300): Promise<string> {
