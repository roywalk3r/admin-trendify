...
export async function uploadFile(
  ...
  try {
    ...
    if (error.message?.includes("CORS") || error.code === 0) {
      toast.error(
        "CORS error: Your application domain is not allowed to access Appwrite"
      );
    } else if (error.code === 401) {
      toast.error("Authentication error: Please log in to upload files");
    } else if (error.code === 403) {
      toast.error(
        "Permission denied: You do not have permission to upload files"
      );
    } else {
...
export function isAppwriteUrl(url: string): boolean {
  try {
    const urlObj = new URL(url);
    return (
      urlObj.hostname.includes("appwrite") &&
...
export async function testAppwriteConnection(): Promise<{
  ...
  try {
    ...
    if (error.code === 0 || error.message?.includes("CORS")) {
      ...
      details = {
        suggestion:
          "Add your application domain to the Appwrite project platform settings",
      };
    } else if (error.code === 401) {
      errorType = "Authentication";
      details = {
        suggestion:
          "Make sure you are logged in or the resource is publicly accessible",
      };
    } else if (error.code === 403) {
      errorType = "Permission";
      details = {
        suggestion: "Check that your user role has the required permissions",
        missingScope: error.message?.includes("missing scope")
          ? error.message
          : "Unknown",
      };
      ...
      details = {
        suggestion:
          "Verify that the bucket ID exists and is correctly configured",
      };
    }

    return {
      success: false,
      message: `Connection failed: ${errorType} error - ${
